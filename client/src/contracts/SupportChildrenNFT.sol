...
contract SupportChildrenNFT is ISupportChildrenNFT, ERC721 {
    ...
    function mint(address to, string calldata receipt) override external {
        ...
        // _setNFTUri(supportChildren.donorsFirstCampaign, nextTokenId++);
        _setNFTUri(receipt, nextTokenId++);

    }

    function _setNFTUri(string memory receipt, uint256 _tokenId) internal {            
        // ISupportChildren.Campaign memory campaign = supportChildren.getCampaign(_campaignId);
        _setTokenURI(_tokenId, receipt);
    }
